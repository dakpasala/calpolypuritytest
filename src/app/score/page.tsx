...
export default function ScorePage() {
  ...
  const getReactionText = (score: number) => {
    if (score >= 90) return "You genuinely do not got to SLO bruh 😭";
    if (score >= 75) return "You’ve had a real Cal Poly experience! 💚💛";
    if (score >= 50) return "Definitely been around SLO a bit 👀";
    if (score >= 25) "Yeah… you’re cooked 😵‍💫";

    return "Huh.. no words can explain your behavior 🫠"
  };

  return (
    <main className="min-h-screen bg-yellow-100 flex justify-center items-start px-4 py-10">
      <div className="bg-[#fff9ec] p-8 rounded-lg shadow-lg max-w-4xl w-full">
        ...
        <div className="flex justify-center gap-4">
          <a
            href={`https://twitter.com/intent/tweet?text=I+scored+${score}%25+on+the+Cal+Poly+SLO+Purity+Test!+https://your-site.com`}
            target="_blank"
          ...
          <a
            href={`https://api.whatsapp.com/send?text=I+scored+${score}%25+on+the+Cal+Poly+SLO+Purity+Test!+https://your-site.com`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={handleSocialShare}
